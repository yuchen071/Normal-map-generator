# -*- coding: utf-8 -*-
"""
Created on Tue Jun  8 16:43:43 2021

@author: Eric
"""

from model import Unet
from utils import random_fliplr, random_crop, gray2rgb
...
class TrainDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        target = Image.open(target_filename).convert('L')
        img = transform(img)
        target = gray_transform(target)
...
def train(img_folder, label_folder, name_list, valid_folder, pretrained=None):
    ...
    for epoch in range(start_epoch, PARAMS["train"]["epochs"]):
        ...
        if (epoch+1) % PARAMS["train"]["log_interv"] == 0 or epoch == 0:
            with torch.no_grad():
                ...
                target = gray2rgb(target).to(device)
    ...
    torch.save({
        ...
        }, os.path.join(CHK_OUT, f"disp_net_epoch_{epoch+1:03}.pth"))
...
def main():
    ...
    disp_folder = os.path.join(DIR_TRAIN, "displacement")
    ...
    test_disp_folder = os.path.join(DIR_TEST, "output_disp")
    valid_folder = os.path.join(DIR_VALID, "disp")
    if not os.path.exists(test_disp_folder):
        os.makedirs(test_disp_folder)
    if not os.path.exists(valid_folder):
        os.makedirs(valid_folder)

    disp_net = train(color_folder, disp_folder, name_list, valid_folder, pretrained=PARAMS["pretrain"])
    test(disp_net, test_in_folder, test_disp_folder)
    if str(device) == 'cuda':
